...
console.log("xxxxxxxxxxxxxxx", TEMPLATES_DIR);
...
exports.deploy = function(bundlePath, env, config) {
  ...
  var dockerImage = config.docker_img_fast;
  var dockerParam = config.docker_param;
  ...
  deployAndVerify(taskList, appName, env.PORT, deployCheckWaitTime ,config.meteor_container_port, dockerImage, dockerParam);
...
function deployAndVerify(taskList, appName, port, deployCheckWaitTime, meteor_container_port, docker_img, docker_param) {
  // deploying
